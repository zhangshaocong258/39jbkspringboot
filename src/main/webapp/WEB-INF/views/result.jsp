...
<head>
    ...
    <script>
        function handler() {
            $.get("resultAjax", $("#zz").serialize(), function (data) {
                $("#resultdiv").html("");
                $("#zhengzhuangdiv").html("");
                $("#inzhengzhuangdiv").html("");
                $.each(data[0]['results'], function (index, obj) {
                    $("#resultdiv").append("<div class='row'> <h3><strong>" + obj['name'] + "</strong></h3> </div>" +
                            "<div class='row'> <p>&#12288;&#12288;" + obj['description'] + "</p> </div>" +
                            "<div class='row'> <h6 style='color:#999999;'>可能性:&thinsp;" + obj['pro'] + "</h6> </div> " +
                            "<div class='row'> <hr> </div>");
                });
                $.each(data[1]['existZhengzhuangs'], function (index, obj) {
                    $("#zhengzhuangdiv").append("<tbody>");
                    if (index == "existZhengzhuangs0") {
                        $("#zhengzhuangdiv").append("<tr> <td>&#12288;<input type='checkbox' name='zhengzhuang' value= '" + obj + "' onclick='return false;' checked>&thinsp;&thinsp;" + obj + "</td></tr>");
                    }
                    else {
                        $("#zhengzhuangdiv").append("<tr> <td>&#12288;<input type='checkbox' name='zhengzhuang' value= '" + obj + "' checked>&thinsp;&thinsp;" + obj + " </td> </tr>");
                    }
                    $("#zhengzhuangdiv").append("</tbody>");

                });

                $.each(data[2]['inexistZhengzhuangs'], function (index, obj) {
                    $("#inzhengzhuangdiv").append("<tbody>");
                    $("#inzhengzhuangdiv").append("<tr> <td>&#12288;<input type='checkbox' name='zhengzhuang' value='" + obj+"' >&thinsp;&thinsp;" + obj + " </td> </tr>");
                    $("#inzhengzhuangdiv").append("</tbody>");
                });
            }, "json");
        }
...
<div style="margin:0 auto; margin-top: 80px; width:970px">
    <div class="box">
        ...
        <form id="zz" class="form-horizontal" action="" method="get">
            <div style="position:absolute;left:92px;top:100px">
                <select id="disabledSelect" name="gender" value="${gender}" class="form-control" style="width:80px;">
                    <option<c:if test="${gender == '男'}"> selected = "selected"</c:if>>男</option>
                    <option<c:if test="${gender == '女'}"> selected = "selected"</c:if>>女</option>
                </select>
            </div>
            <%--<div style="position:absolute;left:92px;top:100px">--%>
                <%--<button name="gender" value="${gender}" class="btn" style="width:80px;">--%>
                <%--</button>--%>
            <%--</div>--%>
        ...
        <%--<form id="zz" class="form-horizontal" action="" method="get">--%>
            ...
            <div  style="overflow:auto;position:absolute;left:3px;top:195px;width:220px;height:206px">
                <table id="zhengzhuangdiv" class="table table-condensed">
                    <tbody>
                        <c:forEach items="${existZhengzhuangs}" var="existZhengzhuang" begin="0" end="0">
                            <tr >
                                <td>&#12288;<input type="checkbox" name="zhengzhuang" value="${existZhengzhuang}"
                                                   onclick="return false;" checked>&thinsp;&thinsp;${existZhengzhuang}
                                </td>
                            </tr>
                        </c:forEach>
                        <c:forEach items="${existZhengzhuangs}" var="existZhengzhuang" begin="1">
                            <tr>
                                <td>&#12288;<input type="checkbox" name="zhengzhuang" value="${existZhengzhuang}"
                                                   checked>&thinsp;&thinsp;${existZhengzhuang}</td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
            ...
            <div style="overflow:auto;position:absolute;left:3px;top:437px;width:220px;height:340px">
                <table id="inzhengzhuangdiv"  class="table table-condensed">
                    <tbody>
                    <div id="inexistZhengzhuangsdiv">
                        <c:forEach items="${inexistZhengzhuangs}" var="inexistZhengzhuang">
                            <tr>
                                <td>&#12288;<input type="checkbox" name="zhengzhuang"
                                                   value="${inexistZhengzhuang}">&thinsp;&thinsp;${inexistZhengzhuang}
                                </td>
                            </tr>
                        </c:forEach>
                    </div>
            ...
            <div style="position:absolute;left:154px;top:168px">
                <button class="btn btn-primary btn-xs" type="button" onclick="handler()">重新检查</button>
            </div>
